...
gene.only.positive.strand <- subset(df.positive.strand, grepl("NYNRIN", rownames(df.positive.strand)))
gene.only.negative.strand <- subset(df.negative.strand, grepl("NYNRIN", rownames(df.negative.strand)))
...
df.final <- subset(df.final, start.pos != -1)
...
plot.gene <- function(df.input, isoform.input, pos.or.neg.logfc) {
  ...
  for (i in 1:nrow(isoform.input)) {
     p <- p + geom_segment(x=isoform.input$start.relative[i], y= -0.4, 
                           xend=isoform.input$end.relative[i], yend= -0.4,
                           size=5,
                           color="gray")
  }
